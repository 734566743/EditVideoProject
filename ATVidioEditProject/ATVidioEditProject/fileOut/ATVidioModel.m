...
@interface ATVidioModel ()<AVCaptureFileOutputRecordingDelegate>
...
@property (nonatomic, assign) ATFlashState flashState;
@property (nonatomic, assign) ATVidioType soundType;

@end

@implementation ATVidioModel
...
- (void)setRecordState:(ATRecordState)recordState
{
...
#pragma mark - setup
- (void)setUpWithType:(ATVideoViewType )type
{
    ...
    ///2. 设置音频的输入
    if (_soundType == ATVidioSoundType) {
        [self setUpAudio];
    }
...
- (void)switchflash
{
    if(_flashState == ATFlashClose){
        if ([self.videoInput.device hasTorch]) {
            ...
            _flashState = ATFlashOpen;
        }
    }else if(_flashState == ATFlashOpen){
        if ([self.videoInput.device hasTorch]) {
            [self.videoInput.device lockForConfiguration:nil];
            [self.videoInput.device setTorchMode:AVCaptureTorchModeAuto];
            [self.videoInput.device unlockForConfiguration];
            _flashState = ATFlashAuto;
        }
    }else if(_flashState == ATFlashAuto){
        if ([self.videoInput.device hasTorch]) {
            [self.videoInput.device lockForConfiguration:nil];
            [self.videoInput.device setTorchMode:AVCaptureTorchModeOff];
            [self.videoInput.device unlockForConfiguration];
            _flashState = ATFlashClose;
        }
    };
...
- (void)reset
{
    self.recordState = ATRecordStateInit;
    _recordTime = 0;
    [self.session startRunning];
}
...
- (void)setUpInit
{
    ...
    [self clearFile];
    _recordTime = 0;
    _recordState = ATRecordStateInit;
...
{
    self.recordState = ATRecordStateRecording;
...
- (void)captureOutput:(AVCaptureFileOutput *)captureOutput didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL fromConnections:(NSArray *)connections error:(NSError *)error
{
    ...
    if ([XCFileManager isExistsAtPath:[self.videoUrl path]]) {
        ...
        self.recordState = ATRecordStateFinish;
        //剪裁成正方形
